...
import { Listing, ListingUpdateRequest } from '../../core/models/listing.model';
...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 class="text-2xl font-bold text-gray-900 mb-8">Modifier l'annonce</h1>
      ...
      @if (loading()) {
        ...
        <form (ngSubmit)="onSubmit()" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" [(ngModel)]="form.pricePerNight" name="pricePerNight" min="1"
                       [class]="inputClass((form.pricePerNight ?? 0) < 1)"/>
              </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" [(ngModel)]="form.maxGuests" name="maxGuests" min="1"
                       [class]="inputClass((form.maxGuests ?? 0) < 1)"/>
              </div>
              <div>
          ...
          <div class="bg-white border border-gray-200 rounded-xl p-6 space-y-4"
               [class.border-red-400]="submitted() && editImages.length === 0">
            ...
            @if (editImages.length > 0) {
              <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                @for (img of editImages; track $index) {
          ...
          @if (success()) {
            <p class="text-sm text-green-600">Annonce mise à jour !</p>
          }

          <div class="flex gap-3">
            <button type="submit" [disabled]="submitting()"
                    class="flex-1 py-3 bg-rose-500 text-white rounded-lg font-medium hover:bg-rose-600 transition disabled:opacity-50">
              @if (submitting()) { Sauvegarde... } @else { Sauvegarder }
            </button>
...
export class EditListingComponent implements OnInit {
  ...
  form: ListingUpdateRequest = {};
  editImages: { imageUrl: string; primary: boolean; sortOrder: number }[] = [];
    ...
    if (id) {
      this.listingService.getById(id).subscribe({
        next: (l) => {
          ...
          if (l.latitude && l.longitude) this.addressVerified.set(true);
          this.editImages = (l.images || []).map((img, i) => ({
      ...
      this.http.post<{ url: string }>(`${environment.apiUrl}/listings/upload`, formData).subscribe({
        next: (res) => {
          this.editImages.push({
            imageUrl: res.url,
            primary: this.editImages.length === 0,
            sortOrder: this.editImages.length,
          });
          uploaded++;
  ...
  addImageUrl() {
    this.editImages.push({ imageUrl: '', primary: this.editImages.length === 0, sortOrder: this.editImages.length });
  }

  removeImage(index: number) {
    this.editImages.splice(index, 1);
    this.editImages.forEach((img, i) => { img.sortOrder = i; img.primary = i === 0; });
  }
  ...
  onSubmit() {
    ...
    if ((this.form.pricePerNight ?? 0) < 1) {
      this.error.set('Le prix par nuit doit être supérieur à 0.');
      return;
    }
    if ((this.form.maxGuests ?? 0) < 1) {
      this.error.set('Le nombre de voyageurs doit être supérieur à 0.');
      return;
    }

    const validImages = this.editImages.filter((img) => img.imageUrl.trim() !== '');
    ...
    const doUpdate = () => {
      const payload: ListingUpdateRequest = {
        ...this.form,
        images: this.editImages.filter((img) => img.imageUrl.trim() !== ''),
      };
      this.listingService.update(l!.id, payload).subscribe({
        next: (updated) => {
  ...
  goBack() {
    const l = this.listing();
    if (l) {
      this.router.navigate(['/listing', l.id]);
    } else {
